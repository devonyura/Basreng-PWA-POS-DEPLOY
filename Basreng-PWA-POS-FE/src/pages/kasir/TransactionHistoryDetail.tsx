...
const TransactionHistoryDetail: React.FC<TransactionHistoryDetailProps> = ({
  ...
  const autoGeneratedTransactionRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (!isOpen) {
      autoGeneratedTransactionRef.current = null;
      return;
    }

    const loadedTransactionCode =
      transactionData?.transactions.transaction_code ?? null;

    if (
      !autoGenerateReceipt ||
      !transactionCode ||
      !transactionData ||
      loadedTransactionCode !== transactionCode ||
      autoGeneratedTransactionRef.current === transactionCode
    ) {
      return;
    }
  ...
  }, [autoGenerateReceipt, isOpen, transactionCode, transactionData]);
